...
require_once __DIR__ . '/src/StockService.php'; // 假設你已將測試代碼封裝成 Service

$pdo = Database::getInstance()->getConnection();
$assetService = new AssetService($pdo);
$stockService = new StockService(); // 包含 getPrice($symbol) 邏輯

echo "--- Starting Stock Price Sync: " . date('Y-m-d H:i:s') . " ---\n";

// 1. 找出所有具備 symbol 與數量，且類型為股票或債券的帳戶
$sql = "SELECT id, user_id, name, type, symbol, quantity, currency_unit 
        FROM accounts 
...
foreach ($stockAccounts as $acc) {
    ...
    if ($currentPrice !== null) {
        ...
        $success = $assetService->upsertAccountBalance(
            ...
            $acc['currency_unit'],
            date('Y-m-d'), // 今日
            null,          // ledgerId 由 Service 自動處理或維持原狀
            null,          // customRate
            $symbol,
            (float)$acc['quantity']
        );
